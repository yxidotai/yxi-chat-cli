...
## Playwright UI 测试 agent

### 1) Docker 方式运行 Playwright MCP
...
- `case_id` / `case` / `test_id`：用例编号
- `title` / `name`：用例标题
- `action`：步骤动作（goto/click/fill/press/expect_text/expect_visible/wait/screenshot）
- `selector`：元素定位（CSS/XPath）
- `value`：输入值或断言文本
- `url`：访问 URL（可为相对路径，会与 `--base-url` 组合）
- `wait_ms`：等待毫秒
...
- `test-case.xlsx` 必须是 Playwright MCP 容器可见路径（如 `/data`）。
- 若需有界面运行，可加 `--headed`。
- 运行完成后，结果会以 JSON 形式输出。
